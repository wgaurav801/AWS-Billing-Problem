...
namespace BillingEngine.DomainModelGenerators
{
    public class Ec2InstanceTypeDomainModelGenerator
    {
        ...
        {
            var sc = parsedEc2InstanceTypes.Select(record => generateEc2InstanceType(record.Ec2InstanceType, double.Parse(record.CostPerHourOnDemand[1..]),record.Region, ec2Regions)).ToList();
            // Convert each object of type ParsedEc2InstanceType to Ec2InstanceType

            return sc;
        }

        private Ec2InstanceType generateEc2InstanceType(string Ec2InstanceType, double CostPerHour, string Region, List<Ec2Region> ec2Regions)
        {
           // Ec2InstanceType sc = new Ec2InstanceType(Ec2InstanceType, CostPerHour,Region);
            foreach (var ec2Region in ec2Regions)
            {
                ...
                {
                    return new Ec2InstanceType(Ec2InstanceType, CostPerHour, Region,true); 
                }
                else
                {
                }
               ...
               return new Ec2InstanceType(Ec2InstanceType, CostPerHour, Region,false);
        }
